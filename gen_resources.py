...
def masthead():
    items=[("Foundation","foundation/index.html"),("Research","research/index.html"),
           ("Management","management/index.html"),("All courses","courses/index.html"),
...
def footer():
  ...
  <div class="wrap">
    <div class="cols">
      ...
      <div><h5>Resources</h5><a href="toolkit.html">Free Tools Directory</a><a href="library.html">Reading Library</a><a href="courses/index.html">All courses</a></div>
...
def shell(title,desc,head_crumb,h1,sub,body):
...
{masthead()}
<main id="main">
<div class="lessonhead"><div class="wrap">
  <p class="crumb">{head_crumb}</p>
...
def deflist(rows):
    r="".join(f'<div class="row"><b>{html.escape(n)}</b><span>{html.escape(d)}</span><span><a href="{html.escape(u)}">{html.escape(u.replace("https://","").replace("http://","").rstrip("/"))}</a></span></div>' for n,d,u in rows)
...
with open(os.path.join(ROOT,"toolkit.html"),"w") as f:
    f.write(shell("Free Tools Directory — Toolkit","Every tool with a usable free tier, for the hands-on activities.",
                  "Resources","Free Tools Directory","Curated free tools for every hands-on activity",tk_body))

# ===== LIBRARY =====
def reflist(items):
    r="".join(f'<div class="pub">{html.escape(n)}{(" · <a href=\""+u+"\">link ↗</a>") if u else ""}</div>' for n,u in items)
    return r
...
with open(os.path.join(ROOT,"library.html"),"w") as f:
    f.write(shell("Reading & Resource Library","The papers, books, repos, and courses behind the curriculum.",
                  "Resources","Reading &amp; Resource Library","Papers, books, repos &amp; courses behind the curriculum",lib_body))
